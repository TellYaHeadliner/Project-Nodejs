...
const { getBlog, getBlogDetail, getBlogs, findBlog } = require('../middleware/getBlog')
...
routerGeneral.get('/signin', async (req,res) => {
    res.render('general/main_signin')
})
...
routerGeneral.get('/pages/', async(req,res) => {
    ...
    try {
        ...
        const header = await getHeader()
        const footer = await getFooter()
        res.render('general/general', {result: footer, chuDe: header, blogs : blog ,user: user, header: '../general/header', footer: '../general/footer', main: '../general/main'});
    } catch (error) {
        console.error(error)
        res.status(500).json({message: "Lỗi server"})
    }
...
routerGeneral.get('/search', async (req, res) => {
    ...
    const blog = await findBlog(search);
    console.log(blog);
    req.session.user = null;
    const user = req.session.user;
    const header = await getHeader();
